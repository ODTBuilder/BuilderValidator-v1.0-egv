...
gitbuilder.ui.NewGeneralizationV2Window = function NewGeneralizationV2Window(flag) {
	if (!gitbuilder.variable.elementid.generalizationV2Window) {
		...
		var generDiaContent = "<div class='modal fade' id='" + generalizationV2WindowId
				+ "' tabindex='-1' data-focus-on='input:first' role='dialog'>";
...
gitbuilder.ui.NewFileWindow = function NewFileWindow() {
	if (!gitbuilder.variable.elementid.fileWindow) {
		...
		/*
		 * fileWindow += '<div class="col-lg-12 text-right">'; fileWindow += '<label>Upon
		 * duplicate approval, The existing file is overwritten.</label>';
		 * fileWindow += '</div>';
		 */
		...
		// 중복파일
		/*
		 * fileWindow += '<div class="col-lg-3 text-right checkbox
		 * checkbox-secondary checkbox-single" style="padding-right:0px">';
		 * fileWindow += '<input type="checkbox" data-toggle="tooltip"
		 * data-placement="bottom" title="Upon duplicate approval, The existing
		 * file is overwritten." id="dupchk"><label>Duplicate approvals</label><br>';
		 * fileWindow += '</div>';
		 */
		...
		var myDropzone = new Dropzone(document.body, { // Make the whole body a
			// dropzone
			url : "file/fileUpload.do", // Set the url
			thumbnailWidth : 80,
			thumbnailHeight : 80,
			parallelUploads : 20,
			maxFiles : 100,
			...
			acceptedFiles : ".zip",
			autoQueue : false, // Make sure the files aren't queued until
			// manually added
		...
		myDropzone.on("addedfile", function(file) {
			if (!file.type.match(/image.*/)) {
				...
				if (/dxf$/.test(beforestr)) {
					myDropzone.emit("thumbnail", file, "resources/img/dxf.png");
				}
				if (/ngi$/.test(beforestr)) {
					myDropzone.emit("thumbnail", file, "resources/img/ngi.png");
				}
				if (/nda$/.test(beforestr)) {
					myDropzone.emit("thumbnail", file, "resources/img/nda.png");
				}
				if (/zip$/.test(beforestr)) {
					myDropzone.emit("thumbnail", file, "resources/img/shp.png");
				}
			} else {
...
/*
 * $(document).on("change","#dupchk",function () { var ischecked =
 * $('input:checkbox[id="dupchk"]').is(":checked"); if(ischecked){ var content
 * ={}; var confirmFunc; content = { title : 'File duplication approved.', text :
 * 'Upon duplicate approval, The existing file is overwritten.', type :
 * 'warning', confirmButtonColor : '#3085d6', confirmButtonText : 'OK',
 * confirmButtonClass : 'btn btn-danger', buttonsStyling : false }; confirmFunc =
 * function(isConfirm){ if (isConfirm) {
 * gitbuilder.variable.myDropzone.options.duplicate = true;
 * gitbuilder.variable.myDropzone.initFileTypeChange(); //파일리스트 초기화 } }
 * alertConfirmPopup(content,confirmFunc); } else{
